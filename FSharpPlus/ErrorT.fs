...
type ErrorT with
    ...
    static member inline Tell (w:'Monoid) = w |> Writer.tell |> lift : ErrorT<Writer<'Monoid,Choice<unit,'E>>>
    ...
    static member inline Pass m = ErrorT (ErrorT.run m >>= choice (result << Choice2Of2) (map Choice1Of2 << Writer.pass << result)) : ErrorT<Writer<'Monoid,Choice<'T,'E>>>

    static member get_Get()  = lift State.get         : ErrorT<State<'S,Choice<_,'E>>>
    static member Put (x:'S) = x |> State.put |> lift : ErrorT<State<'S,Choice<_,'E>>>