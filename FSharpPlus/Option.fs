...
type OptionT with
    ...
    static member inline MZero (_output: OptionT<'``MonadPlus<option<'T>``>, _impl:MZero) = OptionT <| result None                                                               : OptionT<'``MonadPlus<option<'T>``>
    static member inline MPlus (OptionT x, OptionT y, _impl:MPlus)                        = OptionT <| (x  >>= (fun maybe_value -> match maybe_value with Some _ -> x | _ -> y)) : OptionT<'``MonadPlus<option<'T>``>
