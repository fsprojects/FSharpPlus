...
type ReaderT with
    ...
    static member Tell   w           = w |> Writer.tell |> lift         : ReaderT<'R, Writer<'Monoid,unit>>
    ...
    static member get_Get() = lift State.get         :ReaderT<'R, State<'S, 'S>>
    static member Put x     = x |> State.put |> lift :ReaderT<'R, State<'S, unit>>