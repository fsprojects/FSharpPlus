...
type StateT<'s, 'monad, 't> with
    ...
    static member inline TryWith (source: unit -> StateT<'S, 'Monad, 'T>, f: exn -> StateT<'S, 'Monad, 'T>) =
        StateTOperations.StateT< '``Monad<'T * 'S>``, 'Monad, 'S, 'T> (fun s -> TryWith.Invoke  (fun () -> (StateT.run (source ()) s : '``Monad<'T * 'S>`` )) (fun x -> StateT.run (f x) s))

    static member inline TryFinally (computation: unit -> StateT<'S,'Monad,'T>, f) =
        StateTOperations.StateT< '``Monad<'T * 'S>``, 'Monad, 'S, 'T> (fun s -> TryFinally.Invoke (fun () -> StateT.run (computation ()) s) f)
