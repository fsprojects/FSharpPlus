...
type NonEmptyList<'t> =
    ...
    interface System.Collections.IEnumerable with
        member x.GetEnumerator() =
            (let { Head = x; Tail = xs } = x in seq (x :: xs)).GetEnumerator() :> System.Collections.IEnumerator

    interface IReadOnlyCollection<'t> with
        member s.Count = 1 + List.length s.Tail

    interface IReadOnlyList<'t> with
        member s.Item
            with get index = s.Item index
...
module NonEmptyList =
    ...
    let mapi f { Head = x; Tail = xs } =
        let maperI = (fun i item -> f (i + 1) item)
        { Head = f 0 x
          Tail = List.mapi maperI xs }

    /// <summary>Splits a list of pairs into two lists.</summary>
    /// <param name="list">The input list.</param>
    /// <returns>Two lists of split elements.</returns>
    let unzip (list: NonEmptyList<'T1 * 'T2>) =
        let t1, t2 = List.unzip list.Tail in { Head = fst list.Head; Tail = t1 }, { Head = snd list.Head; Tail = t2 }
    ...
    let zip (list1: NonEmptyList<'T>) (list2: NonEmptyList<'U>) =
        { Head = (list1.Head, list2.Head)
          Tail = List.zip list1.Tail list2.Tail }
    ...
    let inline traverse (f: 'T -> '``Functor<'U>``) (s: NonEmptyList<'T>) =
        let lst: '``Functor<'List<'U>>`` = traverse f (toList s)
        (create
         << List.head
         |> fun f x -> f x (List.tail x))
        <!> lst: '``Functor<NonEmptyList<'U>>``
#endif
    ...
    /// <param name="list">The input list.</param>
    /// <returns>The resulting average.</returns>
    let inline averageBy (projection: 'T -> ^U) list =
        List.averageBy projection (list.Head :: list.Tail)
    ...
    let reduce (reduction: 'T -> 'T -> 'T) list =
    ...
    let reduceBack (reduction: 'T -> 'T -> 'T) list =
    ...
    let inline choice (list: NonEmptyList<'``Alt<'T>``>): '``Alt<'T>`` = reduce (<|>) list
#endif
...
type NonEmptyList<'t> with
    ...
    static member (>>=)({ Head = x; Tail = xs }, f: _ -> NonEmptyList<'b>) =
        ...
        let ys' =
            List.collect (NonEmptyList.toList << f) xs
    ...
    static member (<*>)(f: NonEmptyList<'T -> 'U>, x: NonEmptyList<'T>) =
        let r =
            NonEmptyList.toList f
            </ List.apply
            /> NonEmptyList.toList x

        { Head = r.Head; Tail = r.Tail }

    static member Lift2(f: 'T -> 'U -> 'V, x, y) =
        NonEmptyList.ofList (List.lift2 f (NonEmptyList.toList x) (NonEmptyList.toList y))

    static member Extract { Head = h; Tail = _ }: 't = h

#if !FABLE_COMPILER
    static member Duplicate(s: NonEmptyList<'a>, [<Optional>] _impl: Duplicate) = NonEmptyList.tails s
#endif

    static member (=>>)(s, g): NonEmptyList<'b> =
        NonEmptyList.map g (NonEmptyList.tails s)


    static member (+)({ Head = h; Tail = t }, x) =
        { Head = h
          Tail = t @ NonEmptyList.toList x }
    ...
    [<EditorBrowsable(EditorBrowsableState.Never)>]
    static member inline Traverse(s: NonEmptyList<'T>, f: 'T -> '``Functor<'U>``): '``Functor<NonEmptyList<'U>>`` =
        NonEmptyList.traverse f s

    static member Replace(source: NonEmptyList<'T>,
    ...
    static member inline Choice(source: NonEmptyList<'``Alt<'T>``>) =
        use e =
            (NonEmptyList.toSeq source).GetEnumerator()
...
module NonEmptyListBuilder =
    type NelBuilder() =
        ...
        member __.Delay expr: NonEmptyList<'T> = expr ()
