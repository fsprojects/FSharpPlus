...
type OptionT<'``monad<option<'t>>``> with
    ...
    static member inline TryWith (source: unit -> OptionT<'``Monad<option<'T>>``>, f: exn -> OptionT<'``Monad<option<'T>>``>) = OptionT (TryWithS.InvokeFromOtherMonad (fun () -> OptionT.run (source ())) (OptionT.run << f))
    static member inline TryFinally (computation: unit -> OptionT<'``Monad<option<'T>>``>, f) = OptionT (TryFinallyS.Invoke (fun () -> OptionT.run (computation ())) f)
