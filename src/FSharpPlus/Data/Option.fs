...
type OptionT<'``monad<option<'t>>``> with
    ...
    static member inline TryWith (source: OptionT<'``Monad<option<'T>>``>, f: exn -> OptionT<'``Monad<option<'T>>``>) = OptionT (TryWith.InvokeForStrict (fun () -> OptionT.run source) (OptionT.run << f))
    static member inline TryFinally (computation: OptionT<'``Monad<option<'T>>``>, f) = OptionT (TryFinally.InvokeForStrict (fun () -> OptionT.run computation) f)
