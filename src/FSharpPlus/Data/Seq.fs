...
type SeqT<'``monad<seq<'t>>``> with
    ...
    static member inline TryWith (source: unit -> SeqT<'``Monad<seq<'T>>``>, f: exn -> SeqT<'``Monad<seq<'T>>``>) = SeqT (TryWithS.InvokeFromOtherMonad (fun () -> SeqT.run (source ())) (SeqT.run << f))
    static member inline TryFinally (computation: unit -> SeqT<'``Monad<seq<'T>>``>, f) = SeqT (TryFinallyS.Invoke (fun () -> SeqT.run (computation ())) f)
