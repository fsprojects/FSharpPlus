...
type Apply =
    ...
    static member        ``<*>`` (f: Lazy<'T->'U>     , x: Lazy<'T>             , _output: Id<Lazy<'U>>             , [<Optional>]_mthd: Apply) = Lazy<_>.Create (fun () -> f.Value x.Value)   : Lazy<'U>
    static member        ``<*>`` (f: seq<_>           , x: seq<'T>              , _output: Id<seq<'U>>              , [<Optional>]_mthd: Apply) = Seq.apply  f x                               : seq<'U>
    #if !FABLE_COMPILER
    static member        ``<*>`` (f: IEnumerator<_>   , x: IEnumerator<'T>      , _output: Id<IEnumerator<'U>>      , [<Optional>]_mthd: Apply) = Enumerator.map2 id f x : IEnumerator<'U>
    #endif
    static member        ``<*>`` (f: list<_>          , x: list<'T>             , _output: Id<list<'U>>             , [<Optional>]_mthd: Apply) = List.apply f x                               : list<'U>
    static member        ``<*>`` (f: _ []             , x: 'T []                , _output: Id<'U []>                , [<Optional>]_mthd: Apply) = Array.collect (fun x1 -> Array.collect (fun x2 -> [|x1 x2|]) x) f : 'U []
    static member        ``<*>`` (f: 'r -> _          , g: _ -> 'T              , _output: Id< 'r -> 'U>            , [<Optional>]_mthd: Apply) = fun x -> f x (g x)                           : 'U
    static member inline ``<*>`` ((a: 'Monoid, f)     , (b: 'Monoid, x: 'T)     , _output: Id<'Monoid * 'U>         , [<Optional>]_mthd: Apply) = (FSharpPlus.Control.Plus.Invoke a b, f x)                       : 'Monoid *'U
    #if !FABLE_COMPILER
    static member        ``<*>`` (f: Task<_>          , x: Task<'T>             , _output: Id<Task<'U>>             , [<Optional>]_mthd: Apply) = Task.apply   f x : Task<'U>
    #endif
    static member        ``<*>`` (f: Async<_>         , x: Async<'T>            , _output: Id<Async<'U>>            , [<Optional>]_mthd: Apply) = Async.apply  f x : Async<'U>
    static member        ``<*>`` (f: option<_>        , x: option<'T>           , _output: Id<option<'U>>           , [<Optional>]_mthd: Apply) = Option.apply f x : option<'U>
    static member        ``<*>`` (f: Result<_,'E>     , x: Result<'T,'E>        , _output: Id<Result<'b,'E>>        , [<Optional>]_mthd: Apply) = Result.apply f x : Result<'U,'E>
    static member        ``<*>`` (f: Choice<_,'E>     , x: Choice<'T,'E>        , _output: Id<Choice<'b,'E>>        , [<Optional>]_mthd: Apply) = Choice.apply f x : Choice<'U,'E>
    static member inline ``<*>`` (KeyValue(a: 'Key, f), KeyValue(b: 'Key, x: 'T), _output: Id<KeyValuePair<'Key,'U>>, [<Optional>]_mthd: Apply) : KeyValuePair<'Key,'U> = KeyValuePair (Plus.Invoke a b, f x)
    static member inline ``<*>`` (struct (a: 'Monoid, f), struct (b: 'Monoid, x: 'T), _output: Id<struct ('Monoid * 'U)>, [<Optional>]_mthd: Apply) = struct (Plus.Invoke a b, f x)

    static member        ``<*>`` (f: Map<'Key,_>      , x: Map<'Key,'T>         , _output: Id<Map<'Key,'U>>         , [<Optional>]_mthd: Apply) : Map<'Key,'U> = Map (seq {
       for KeyValue(k, vf) in f do
           match Map.tryFind k x with
           | Some vx -> yield k, vf vx
           | _       -> () })

    static member        ``<*>`` (f: Dictionary<'Key,_>, x: Dictionary<'Key,'T> , _output: Id<Dictionary<'Key,'U>>  , [<Optional>]_mthd: Apply) : Dictionary<'Key,'U> =
       let dct = Dictionary ()
       for KeyValue(k, vf) in f do
    ...
    #if !FABLE_COMPILER
    static member        ``<*>`` (f: Expr<'T->'U>, x: Expr<'T>, _output: Id<Expr<'U>>, [<Optional>]_mthd: Apply) = Expr.Cast<'U> (Expr.Application (f, x))
    #endif
    static member        ``<*>`` (f: ('T->'U) ResizeArray, x: 'T ResizeArray, _output: Id<'U ResizeArray>, [<Optional>]_mthd: Apply) =
    ...
    static member inline Invoke (f: '``Applicative<'T -> 'U>``) (x: '``Applicative<'T>``) : '``Applicative<'U>`` =
        ...
        call(Unchecked.defaultof<Apply>, f, x, Unchecked.defaultof<Id<'``Applicative<'U>``>>)
...
type Apply with
    static member inline ``<*>`` (f: '``Monad<'T->'U>``      , x: '``Monad<'T>``      , _output: Id<'``Monad<'U>``>     , _mthd:Default2) : '``Monad<'U>`` = Bind.InvokeOnInstance f (fun (x1: 'T->'U) -> Bind.InvokeOnInstance x (fun x2 -> Return.InvokeOnInstance (x1 x2)))
    static member inline ``<*>`` (_: ^t when ^t : null and ^t: struct, _: ^u when ^u : null and ^u: struct, _output: Id< ^r> when ^r : null and ^r: struct, _mthd: Default1) = id
    static member inline ``<*>`` (f: '``Applicative<'T->'U>``, x: '``Applicative<'T>``, _output: Id<'``Applicative<'U>`` >   , _mthd:Default1) : '``Applicative<'U>`` = Apply.InvokeOnInstance f x
