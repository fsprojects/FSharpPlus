...
type Apply =
    ...
    static member        ``<*>`` (f: Lazy<'T->'U>     , x: Lazy<'T>             , _output: Option<Lazy<'U>>             , [<Optional>]_mthd: Apply) = Lazy<_>.Create (fun () -> f.Value x.Value)   : Lazy<'U>
    static member        ``<*>`` (f: seq<_>           , x: seq<'T>              , _output: Option<seq<'U>>              , [<Optional>]_mthd: Apply) = Seq.apply  f x                               : seq<'U>
    #if !FABLE_COMPILER
    static member        ``<*>`` (f: IEnumerator<_>   , x: IEnumerator<'T>      , _output: Option<IEnumerator<'U>>      , [<Optional>]_mthd: Apply) = Enumerator.map2 id f x : IEnumerator<'U>
    #endif
    ...
    #endif
    static member        ``<*>`` (f: Async<_>         , x: Async<'T>            , _output: Option<Async<'U>>            , [<Optional>]_mthd: Apply) = Async.apply  f x : Async<'U>
    ...
    static member inline ``<*>`` (struct (a: 'Monoid, f), struct (b: 'Monoid, x: 'T), _output: Option<struct ('Monoid * 'U)>, [<Optional>]_mthd: Apply) = struct (FSharpPlus.Control.Plus.Invoke a b, f x)
    ...
    static member        ``<*>`` (f: Dictionary<'Key,_>, x: Dictionary<'Key,'T> , _output: Option<Dictionary<'Key,'U>>  , [<Optional>]_mthd: Apply) : Dictionary<'Key,'U> =
       let dct = Dictionary ()
       for KeyValue(k, vf) in f do
    ...
    #if !FABLE_COMPILER
    static member        ``<*>`` (f: Expr<'T->'U>, x: Expr<'T>, _output: Option<Expr<'U>>, [<Optional>]_mthd: Apply) = Expr.Cast<'U> (Expr.Application (f, x))
    #endif
    static member        ``<*>`` (f: ('T->'U) ResizeArray, x: 'T ResizeArray, _output: Option<'U ResizeArray>, [<Optional>]_mthd: Apply) =
