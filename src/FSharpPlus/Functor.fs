...
type Apply =
    ...
    static member        (<*>) (f: list<_>          , x: list<'T>        ) = List.apply f x                               : list<'U>
    static member        (<*>) (f: _ []             , x: 'T []           ) = Array.collect (fun x1 -> Array.collect (fun x2 -> [|x1 x2|]) x) f : 'U []
    static member        (<*>) (f: 'r -> _          , g: _ -> 'T         ) = fun x -> f x (g x)                           : 'U
    static member inline (<*>) ((a: 'Monoid, f)     , (b: 'Monoid, x: 'T)) = (Plus.Invoke a b, f x)                       : 'Monoid *'U
    #if !FABLE_COMPILER
    static member        (<*>) (f: Task<_>          , x: Task<'T>        ) = Task.apply   f x : Task<'U>
    #endif
    static member        (<*>) (f: Async<_>         , x: Async<'T>       ) = Async.apply  f x : Async<'U>
    static member        (<*>) (f: option<_>        , x: option<'T>      ) = Option.apply f x : option<'U>
    static member        (<*>) (f: Result<_,'E>     , x: Result<'T,'E>   ) = Result.apply f x : Result<'U,'E>
    static member        (<*>) (f: Choice<_,'E>     , x: Choice<'T,'E>   ) = Choice.apply f x : Choice<'U,'E>
    static member inline (<*>) (KeyValue(a: 'Key, f), KeyValue(b: 'Key, x: 'T)) : KeyValuePair<'Key,'U> = KeyValuePair (Plus.Invoke a b, f x)
    ...
    static member        (<*>) (f: Dictionary<'Key,_>, x: Dictionary<'Key,'T>) : Dictionary<'Key,'U> =
       let dct = Dictionary ()
       for KeyValue(k, vf) in f do
    ...
    #if !FABLE_COMPILER
    static member        (<*>) (f: Expr<'T->'U>, x: Expr<'T>) = Expr.Cast<'U> (Expr.Application (f, x))
    #endif
    static member        (<*>) (f: ('T->'U) ResizeArray, x: 'T ResizeArray) =
    ...
    static member inline Invoke (f: '``Applicative<'T -> 'U>``) (x: '``Applicative<'T>``) : '``Applicative<'U>`` =
        let inline call (mthd : ^M, input1: ^I1, input2: ^I2, _output: ^R) =
            ((^M  or ^I2 or ^R or ^I1) : (static member (<*>) : _*_ -> _) input1,input2)
...
type Apply with
    static member        (<*>) (f: Lazy<'T->'U>     , x: Lazy<'T>        ) = Lazy<_>.Create (fun () -> f.Value x.Value)   : Lazy<'U>
    static member        (<*>) (f: seq<_>           , x: seq<'T>         ) = Seq.apply  f x                               : seq<'U>
    #if !FABLE_COMPILER
    static member        (<*>) (f: IEnumerator<_>   , x: IEnumerator<'T> ) = Enumerator.map2 id f x : IEnumerator<'U>
    #endif
    static member inline (<*>) (f: '``Monad<'T->'U>``, x: obj) : '``Monad<'U>`` = Bind.InvokeOnInstance f (fun (x1: 'T->'U) -> Bind.InvokeOnInstance (x :?> '``Monad<'T>``) (fun x2 -> Return.InvokeOnInstance (x1 x2)))
