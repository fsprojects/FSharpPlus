...
module Writer =
    ...
    let inline bind f (Writer (a:'T, w)) = Writer (let (b, w') = run (f a) in (b, Append.Invoke w w'))      : Writer<'Monoid,'U>
    let inline apply  (Writer (f, a)) (Writer (x:'T, b))       = Writer (f x, Append.Invoke a b)            : Writer<'Monoid,'U>
...
type Writer with
    ...
    static member inline Return x = Writer (x, Empty.Invoke())          : Writer<'Monoid,'T>
