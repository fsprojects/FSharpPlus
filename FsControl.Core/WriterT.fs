...
module WriterT =
    ...
    let inline bind (f:'T->WriterT<'``Monad<'U * 'Monoid>``>) (WriterT (m:'``Monad<'T * 'Monoid>``)) = 
        WriterT (m >>= (fun (a, w) -> run (f a) >>= (fun (b, w') -> result (b, MAppend.Invoke w w'))))  : WriterT<'``Monad<'U * 'Monoid>``>
    

type WriterT with

    static member inline Map      (x:WriterT<'``Monad<'T * 'Monoid>``>, f:'T->'U, _:Map) = WriterT.map f x
    static member inline Return   (_:WriterT<'``Monad<'T * 'Monoid>``>, _:Return) :'T -> WriterT<'``Monad<'T * 'Monoid>``> = fun a -> WriterT (result (a, MEmpty.Invoke()))
    ...
    static member inline LiftAsync (output:WriterT<Async<'T * 'Monoid>>) = fun (x: Async<'T>) -> Lift.Invoke (LiftAsync.Invoke x) : WriterT<Async<'T * 'Monoid>>
