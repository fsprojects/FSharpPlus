...
type FoldMap() =
    ...
    static member inline FoldMap (_:Typ1, x:#obj, _) = fun f -> FoldMap.FromFoldr f x
...
type Foldl() =
    ...
    static member inline Foldl (_:Typ1, x    , _) = fun (f,z) -> Foldl.FromFoldMap      f z x
...
type ToList() =
    ...
    static member inline ToList (_:Typ1, x , _) = fun () -> Foldr.Invoke List.cons [] x
...
type ToArray() =
    ...
    static member inline ToArray (_:Typ1, x , _) = fun () -> Foldr.Invoke (fun x y -> Array.concat [[|x|];y]) [||] x
...
type Exists() =
    ...
    static member inline Exists (_:Typ1, x     , _:bool) = fun f -> Foldr.Invoke (fun x b -> f x || b) false x :bool
...
type TryFind() =
    ...
    static member inline TryFind (_:Typ1, x   , _:'T option) = fun f ->
        Foldr.Invoke (fun x b -> (let r = f x in if r then Some x else None) <|> b) None x  :'T option
...
type TryPick() =
    ...
    static member inline TryPick (_:Typ1, x     , _:'U option) = fun (f:_->'U option) -> Foldr.Invoke (fun x b -> f x <|> b) None x  :'U option
