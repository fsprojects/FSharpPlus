...
type ReaderT<'R,'Ma> with
    ...
    static member inline instance (Applicative.Pure, _:ReaderT<'r,'ma>) :'a  -> ReaderT<'r,'ma> = fun a -> 
        ReaderT <| fun _ -> return' a

    static member inline instance (_:Applicative.Apply, f, x, _:ReaderT<'r,'mb>) = fun () -> 
        ReaderT.apply f x :ReaderT<'r,'mb>

    static member inline instance (Monad.Bind  , x, _:ReaderT<'r,'m>) :('b -> ReaderT<'r,'m>) -> ReaderT<'r,'m> = fun f -> 
        ReaderT.bind f x
