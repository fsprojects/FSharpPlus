...
type ReaderT<'R,'Ma> with
    ...
    static member inline Apply  (_:Apply , f, x, _:ReaderT<'r,'mb>) = fun () -> ReaderT.apply f x :ReaderT<'r,'mb>
    static member inline Bind   (_:Bind  , x, _:ReaderT<'r,'m>) :('b -> ReaderT<'r,'m>) -> ReaderT<'r,'m> = fun f -> ReaderT.bind f x

    static member inline Zero (_:Zero, _:ReaderT<_,_>   ) = fun ()          -> ReaderT <| fun _ -> Zero.Invoke()
    static member inline Plus (_:Plus,   ReaderT m   ,_ ) = fun (ReaderT n) -> ReaderT <| fun r -> m r <|> n r
    ...
    static member inline Ask   (_:Ask, _:ReaderT<'r,'a>      ) = fun () -> ReaderT result :ReaderT<'r,'a>
    ...
    static member Listen (_:Listen,   ReaderT m, _:ReaderT<'t,Writer<'a,'b*'a>>) :unit -> ReaderT<'t,Writer<'a,'b*'a>> = fun () -> ReaderT <| fun w -> Writer.listen (m w)  
    static member Pass   (_:Pass  ,   ReaderT m, _:ReaderT<'t,Writer<'a,'b>>   ) :unit -> ReaderT<'t,Writer<'a,'b>>    = fun () -> ReaderT <| fun w -> Writer.pass   (m w)

    static member Get (_:Get    , _:ReaderT<'s,State<'a,'a>>  ) = fun () -> Lift.Invoke (State.get()) :ReaderT<'s,State<'a,'a>>
