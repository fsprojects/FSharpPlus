...
function getMainScriptPath(options, info) {
    ...
    const mainScriptName = info.projectFiles.length > 0 ? getFromFsproj(info) : getFromScript(info);
...
// TODO: Stop after --run?
function findFlagIndex(arr, arg) {
...
function onCompiled(args, opts, info, mustFinish = false) {
    ...
    if (!hasError) {
        ...
        if (isRun) {
            const job = run_1.default(getMainScriptPath(opts, info), runArgs);
            if (mustFinish) {
...
function run(entry, args) {
    ...
    index_1.default(opts).then((info) => {
            ...
            const watcher = chokidarLazy()
                ...
                if (ready && ev === "change") {
                    ...
                    if (!tooClose(filePath, prev)) {
                        ...
                        /// @ts-ignore
                        index_1.default(newOpts, cachedInfo).then((info2) => {
