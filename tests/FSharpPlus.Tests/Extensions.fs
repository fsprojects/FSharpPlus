...
[<Test>]
let ``Map.intersect returns same results independent of the order (associative)`` () = 
...
[<Test>]
let ``Dict.intersect returns same results independent of the order (associative)`` () = 
...
[<Test>]
let ``IReadOnlyDictionary.intersect returns same results independent of the order (associative)`` () = 
