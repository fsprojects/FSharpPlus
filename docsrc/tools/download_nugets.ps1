...
try {
    ...
    & $nuget install FSharp.Formatting.Literate "-ExcludeVersion" "-version" 7.2.9  -source https://www.nuget.org/api/v2 "-OutputDirectory" packages/docs/
    & $nuget install FSharp.Compiler.Service "-ExcludeVersion" "-source" https://www.nuget.org/api/v2 "-OutputDirectory" packages/docs/
    & $nuget install FSharp.Formatting "-ExcludeVersion" "-version" 7.2.9 "-source" https://www.nuget.org/api/v2 "-OutputDirectory" packages/docs/
}
finally {
    Pop-Location
}
